...
const results = JSON.parse(fs.readFileSync(path.join(__dirname, 'output/results_full.json')));
...
const ALL_PAGES = ['homepage','about','features','features-portfolio','features-rebalance','features-round-ups','features-top-ups','features-bam-rewards','features-dca','fees','faq','security','team','blog','podcast','partners','smsf','support','invest','gold-silver','bitcoin-ethereum','waitlist'];
...
for (const name of ALL_PAGES) {
  for (const vp of ['desktop','mobile']) {
    ...
    const hasScreenshots = oldImg && newImg;

    cards += `
    <div class="card">
      <div class="card-header" style="border-left:4px solid ${c}">
        <span class="page-name">${vpIcon} ${name} <span style="font-weight:400;color:#64748b;font-size:12px">(${vp})</span></span>
...
<head>
<meta charset="UTF-8">
<title>Visual Regression Report — Bamboo Website (Full)</title>
<style>
...
<body>
<div class="header">
  <h1>🔍 Visual Regression Report — Bamboo Website (All Pages)</h1>
  <p>Old: <strong>getbamboo.io</strong> → New: <strong>bamboo-web-nextjs.netlify.app</strong> · Full page · ${new Date().toLocaleString()} · ${ALL_PAGES.length} pages × 2 viewports</p>
</div>
<div class="summary">
  <div class="stat"><div class="stat-label">Avg Match</div><div class="stat-value" style="color:#f59e0b">${avg}%</div></div>
  <div class="stat"><div class="stat-label">✅ Passing</div><div class="stat-value" style="color:#22c55e">${passing}</div></div>
  <div class="stat"><div class="stat-label">⚠️ Warning</div><div class="stat-value" style="color:#f59e0b">${warning}</div></div>
  <div class="stat"><div class="stat-label">❌ Failing</div><div class="stat-value" style="color:#ef4444">${failing}</div></div>
</div>
<div class="score-table">
  <table>
    <tr><th>Page</th><th>Desktop</th><th>Mobile</th></tr>
    ${ALL_PAGES.map(name => {
      ...
      return `<tr><td style="font-weight:600">${name}</td><td>${fmt(d)}</td><td>${fmt(m)}</td></tr>`;
    }).join('')}
  </table>
</div>
...
const outPath = path.join(__dirname, 'output/report_full.html');
